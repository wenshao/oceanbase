...
namespace oceanbase
{
  namespace common
  {
    ...
    int kv_to_rowkey(const KV& kv, ObString& rowkey, ObObj* rowkey_obj, int32_t obj_buf_count);
    ...
    public:
      ...
      int del_row(const char* table_name, const ObString& rowkey);

    private:
      bool check_inner_stat() const;
      int add_column(const char* table_name, const ObString& rowkey, const char* column_name, const ObObj& value, ObMutator* mutator);
