...
int ObScanner::RowIterator::get_row_()
{
  ...
  else
  {
    ...
    TBSYS_LOG(DEBUG, "ObScanner::get_row get a cell, row_cell_num_=%ld",
        row_cell_num_);
    row_changed = false;
    while (OB_SUCCESS == err && !row_changed)
    {
      err = get_cell(&cell, &row_changed);
      if (OB_SUCCESS != err && OB_ITER_END != err)
      {
      ...
      {
        if(row_cell_num_ >= OB_MAX_COLUMN_NUMBER)
        {
          err = OB_SIZE_OVERFLOW;
          TBSYS_LOG(WARN, "row cell num is too large:row_cell_num[%ld]", row_cell_num_);
        }
        else
        {
    ...
    if (row_changed)
    {
      TBSYS_LOG(DEBUG, "ObScanner::get_row reach end of row");
    }
    else
    {
      TBSYS_LOG(DEBUG, "ObScanner::get_row reach end of scanner");
    }
  }
  return err;
}
...
first_next_(false), first_next_row_(false), string_buf_()
{
  data_version_ = 0;
  has_range_ = false;
...
int ObScanner::serialize_basic_param(char * buf, const int64_t buf_len, int64_t & pos) const
{
  ...
  if (OB_SUCCESS == ret)
  {
    obj.set_int(is_result_precision_ ? 1 : 0);
    ret = obj.serialize(buf, buf_len, pos);
    if (OB_SUCCESS != ret)
    {
      TBSYS_LOG(WARN, "ObObj serialize error, ret=%d buf=%p data_len=%ld pos=%ld",
    ...
    const bool is_row_changed)
{
  ...
  if (OB_SUCCESS == ret)
  {
        ...
        || cell_info.table_id_ != cur_table_id_)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        TBSYS_LOG(DEBUG, "serialize_table_name_or_id_ error, ret=%d buf=%p buf_len=%ld pos=%ld",
            ret, buf, buf_len, pos);
      }
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
        ...
        || (is_compare_rowkey && cell_info.row_key_ != cur_row_key_))
    {
      ...
      if (OB_SUCCESS != ret)
      {
        TBSYS_LOG(DEBUG, "serialize_row_key_ error, ret=%d buf=%p buf_len=%ld pos=%ld",
                  ret, buf, buf_len, pos);
      }
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
            ...
            || ObActionFlag::OP_DEL_TABLE == cell_info.value_.get_ext()))
    {
      ...
      if (OB_SUCCESS != ret)
      {
        TBSYS_LOG(DEBUG, "ObObj serialize error, ret=%d buf=%p buf_len=%ld pos=%ld",
                  ret, buf, buf_len, pos);
      }
    }
    else if (ObExtendType != cell_info.value_.get_type())
    {
      ...
      if (OB_SUCCESS != ret)
      {
        TBSYS_LOG(DEBUG, "serialize_column_ error, ret=%d buf=%p buf_len=%ld pos=%ld",
                  ret, buf, buf_len, pos);
      }
    }
    else
...
int ObScanner::deserialize_meta_param(const char* buf, const int64_t data_len, int64_t& pos, ObObj &last_obj)
{
  ...
  {
    ObString rowkey;
    ret = deserialize_varchar_(buf, data_len, pos, rowkey, last_obj);
    if (OB_SUCCESS != ret)
    {
    ...
    {
      ret = string_buf_.write_string(rowkey, &last_row_key_);
      rollback_row_key_ = last_row_key_;
    }
  }
...
int ObScanner::deserialize_basic_param(const char* buf, const int64_t data_len, int64_t& pos, ObObj &last_obj)
{
      ...
      else
      {
        ...
        if (ObActionFlag::TABLET_RANGE_FIELD == type)
        {
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            ret = string_buf_.write_string(start_row, &(range_.start_key_));
            ret = string_buf_.write_string(end_row, &(range_.end_key_));
            has_range_ = true;
          }
          // doesn't skip obj here, maybe added some new objs after tablet range
          // but need deserialize next obj
          ret = last_obj.deserialize(buf, data_len, pos);
        }
        else
        {
  ...
  // deserialize is_result_precision_
  if (OB_SUCCESS == ret && ObExtendType != last_obj.get_type())
  {
    ret = last_obj.deserialize(buf, data_len, pos);
    if (OB_SUCCESS == ret && ObIntType == last_obj.get_type())
    {
      ...
      {
        is_result_precision_ = (value == 0 ? false : true);
      }
    }
  }
...
void ObScanner::dump(void) const
{
  ...
  TBSYS_LOG(INFO, "[dump] is_result_precision_=%d", is_result_precision_);
}
