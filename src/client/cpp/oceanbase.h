 ...
 * @brief 从OB_RES结构体中获取全部结果行数
 *
 *   <b>ob_fetch_res_is_precision</b>仅在返回Scan请求结果并且调用
 *   <b>ob_scan_set_precision</b>设置了近似精度时值才是有效的, 
 *   否则, 这里返回的值没有意义.
 *
 *   执行Scan请求时, <b>ob_fetch_res_is_precision</b>函数返回这次查询
 *   是否做了近似操作
 *
 * @param ob_res 有效的指向OB_RES结构体的指针
 * @retval isprecision 如果是近似的结果是0，否则是1
 */
