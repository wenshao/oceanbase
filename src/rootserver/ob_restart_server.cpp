...
int ObRestartServer::restart_servers()
{
  ...
  {
    find_can_restart_server(config_->flag_tablet_replicas_num_.get());
    for(it = server_manager_->begin(); it != server_manager_->end(); ++it)
    {
      if(it->wait_restart_ && it->can_restart_)
      {
        ...
        ret = rpc_stub_->shutdown_cs(chunk_server, true, config_->flag_network_timeout_us_.get());
        if(common::OB_SUCCESS != ret)
        {
...
void ObRestartServer::shutdown_cs(ObServerStatus* it)
{
  ...
  TBSYS_LOG(INFO,"shutdown cs ,monotonic_now:%ld,lease_duration:%ld",monotonic_now, config_->flag_cs_lease_duration_us_.get());
