...
usage()
{
	...
	echo "       $(basename $0) -c 'config_files' -i gateway_index -n gateway_count"
	echo "       $(basename $0) -l config_list_file -i gateway_index -n gateway_count"
	echo "Options:"
	...
	echo "  -t PATH_DATE          the exclusive timestamp for the hadoop output path"
...
global_var_init()
{
	declare log_date=$(date +"%Y%m%d%H%M%S")
	declare pathdate=${SPECIFIED_PATH_DATE}
...
check_config_param()
{
	...
	if [ $RM_DAYS_AGO -lt $DATA_DAYS_AGO ]; then
...
mr_generate_sstable()
{
	...
	declare ts=${SPECIFIED_PATH_DATE}
	declare config_jar="config_${ts}.jar"
	...
	if [ "X$?" != "X0" ]; then
		...
		$hadoop_bin $HADOOP_BIN_CONFIG_PARAM fs -rm $HADOOP_HDFS_PATH$TABLE_CONFIG_DIR$config_jar
		return 2
	else
		$hadoop_bin $HADOOP_BIN_CONFIG_PARAM fs -rm $HADOOP_HDFS_PATH$TABLE_CONFIG_DIR$config_jar
	fi
...
copy_data_from_hadoop()
{
	...
	if [ ${GATEWAY_INDEX} -eq $((GATEWAY_COUNT-1)) ]; then
		declare -i copy_count=$((total_file_count%GATEWAY_COUNT+step_size))
	fi
	...
	do
		...
		if [ $? -ne 0 ]; then
 			#empty sstable file, just touch one empty sstable file, not copy from hadoop
		    	...
		    	file_name=${HADOOP_DATA_DIR}$file_name
		    	${HADOOP_BIN_DIR}hadoop $HADOOP_BIN_CONFIG_PARAM fs -get ${file_name} ${DATA_TMP_DIR}${DISP_FILES[$num]}
		    	if [ "X$?" != "X0" ]; then
				errorlog "ERROR: failed to get file $file_name from hadoop to ${DATA_TMP_DIR}${DISP_FILES[$num]}"
				HADOOP_GET_FAIL_COUNT=$((HADOOP_GET_FAIL_COUNT+1))
				continue
		    	else
				infolog "INFO: get hadoop file: ${file_name} from hadoop to dir: ${DATA_TMP_DIR}${DISP_FILES[$num]}"
		    	fi
		fi
	...
	touch $flag_file
	infolog "INFO: finish copying data from hadoop, succ_count=$HADOOP_GET_SUCC_COUNT, failed_count=$HADOOP_GET_FAIL_COUNT, total_count=$copy_count"
}
...
parpare_scp_data()
{
	...
	if [ -f $flag_file ]; then
		...
		if [ -d $DATA_DISPATCHED_DIR -a "X$(ls $DATA_DISPATCHED_DIR)" != "X" ]; then
			mv ${DATA_DISPATCHED_DIR}* ${DATA_WORK_DIR}
		fi
		...
		if [ ${#files[@]} -eq 0 ]; then
			...
			do
				...
				do
					...
					declare -i ret=$?
					if [ $j -eq 1 ]; then
						dst_uri=${USER_NAME}@${CS_SERVER1[$num]}:${dst_dir}
					elif [ $j -eq 2 ]; then
						dst_uri=${USER_NAME}@${CS_SERVER2[$num]}:${dst_dir}
					else 
						dst_uri=${USER_NAME}@${CS_SERVER3[$num]}:${dst_dir}
					fi
...
remove_old_data()
{
	declare rmdate=$(date -d "$RM_DAYS_AGO day ago" +"%Y-%m-%d")
	declare rmdir=$DATA_LOCAL_DIR$APP_NAME/$rmdate/$TABLE_NAME
	mlog "INFO: remove old data, old_data_dir=$rmdir" $LOG_FILE
	if [ -d $rmdir ]; then
		rm -rf $rmdir
	fi

	if [ -d $DATA_LOCAL_DIR$APP_NAME/$rmdate ]; then
		declare files=$(ls $DATA_LOCAL_DIR$APP_NAME/$rmdate)
		if [ "X$files" = "X" ]; then
			rm -rf $DATA_LOCAL_DIR$APP_NAME/$rmdate
		fi
	fi
	...
	if [ $GATEWAY_INDEX -eq 0 ]; then
		${HADOOP_BIN_DIR}hadoop $HADOOP_BIN_CONFIG_PARAM fs -rmr $HADOOP_DATA_DIR_TOP$APP_NAME/$rmdate/$TABLE_NAME
	fi
}
...
check_result()
{
	declare -i total_file_count=${#DISP_FILES[@]}
	declare -i step_size=$((total_file_count/GATEWAY_COUNT))
	HADOOP_GET_FILE_TOTAL_COUNT=$step_size
	if [ ${GATEWAY_INDEX} -eq $((GATEWAY_COUNT-1)) ]; then
		HADOOP_GET_FILE_TOTAL_COUNT=$((total_file_count%GATEWAY_COUNT+step_size))
	fi

	if [ $HADOOP_GET_FILE_TOTAL_COUNT -ne $SCP_FILE_TOTAL_COUNT -o $SCP_FILE_FAIL_COUNT -gt 0 ]; then
...
schedule_dispatch()
{
	if [ "X$MULT_DISPATCH" != "X" ]; then
		...
		infolog "INFO: gateway_succ_count=$GATEWAY_SUCC_COUNT, gateway_fail_count=$GATEWAY_FAIL_COUNT"
	else
		do_local_dispatch
	fi
}
...
dispatch_tables()
{
	...
	if [ $GATEWAY_INDEX -eq 0 -a $fail_count -eq 0 ]; then
		if [ $GATEWAY_SUCC_COUNT -gt 0 -a $GATEWAY_FAIL_COUNT -eq 0 ]; then
			check_root_table_unchanged
			if [ $? -eq 0 ]; then
				major_freeze
				cur_second=$(date "+%s")
...
do_clean()
{
  DATA_TIMESTAMP_DIR=`dirname $LOCAL_TABLE_DIR`
  rm -rf $DATA_TIMESTAMP_DIR
  infolog "INFO: Remove local data dir $DATA_TIMESTAMP_DIR"
}
...
while getopts "l:c:t:g:i:n:hmbf" option
do
	case $option in
	l)
	...
	c)
		...
		;;
	t)
		SPECIFIED_PATH_DATE=$OPTARG
		if [ "X$TABLE_CONFIG_FILES" == "X" ]; then
			echo "INFO: using self generated timestamp for hadoop output path"
			SPECIFIED_PATH_DATE=$(date -d "$DATA_DAYS_AGO day ago" +"%Y%m%d%H%M%S%N")
		fi
		;;
	g)
