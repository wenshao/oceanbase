...
uint64_t ObMsSqlRequestEvent::id_allocator_ = 0;

ObMsSqlRequestEvent::ObMsSqlRequestEvent(ObMergerLocationCacheProxy * proxy, const ObMergerAsyncRpcStub * rpc) : finish_(false)
{
  timeout_ = 0;
  read_param_ = NULL;
  async_rpc_stub_ = rpc;
...
ObMsSqlRequestEvent::~ObMsSqlRequestEvent()
{
}

int ObMsSqlRequestEvent::init(const uint64_t count, const uint32_t mod_id,  ObSessionManager * session_mgr)
{
  int err = OB_SUCCESS;
...
uint64_t ObMsSqlRequestEvent::get_request_id(void) const
{
  return request_id_;
}

const ObMergerAsyncRpcStub * ObMsSqlRequestEvent::get_rpc(void) const
{
  return async_rpc_stub_;
}

void ObMsSqlRequestEvent::set_timeout(const int64_t timeout)
{
  timeout_ = timeout;
}

void ObMsSqlRequestEvent::set_request_param(const ObReadParam * param, const int64_t timeout)
{
  read_param_ = param;
  timeout_ = timeout;
}

int64_t ObMsSqlRequestEvent::get_timeout(void) const
{
  return timeout_;
}

// create and add a new rpc event to the wait queue
int ObMsSqlRequestEvent::create(ObMsSqlRpcEvent ** event)
{
  int ret = OB_SUCCESS;
  if (NULL == event)
  {
...
int ObMsSqlRequestEvent::destroy(ObMsSqlRpcEvent * event)
{
  int ret = OB_SUCCESS;
  if (NULL == event)
  {
...
int ObMsSqlRequestEvent::signal(ObMsSqlRpcEvent & event)
{
...
const ObReadParam * ObMsSqlRequestEvent::get_request_param(int64_t & timeout) const
{
  timeout = timeout_;
  return read_param_;
}

int ObMsSqlRequestEvent::process_rpc_event(const int64_t timeout, ObMsSqlRpcEvent * event, bool & finish)
{
  bool is_valid = false;
...
int ObMsSqlRequestEvent::remove_wait_queue(ObMsSqlRpcEvent * event, bool & is_valid)
{
  is_valid = false;
  int ret = OB_SUCCESS;
  if (NULL == event)
...
int ObMsSqlRequestEvent::wait(const int64_t timeout)
{
  int ret = OB_SUCCESS;
...
int ObMsSqlRequestEvent::release_succ_event(uint64_t & event_count)
{
  int ret = OB_SUCCESS;
...
int ObMsSqlRequestEvent::create_rpc_event(ObMsSqlRpcEvent ** event)
{
  int ret = OB_SUCCESS;
...
void ObMsSqlRequestEvent::destroy_rpc_event(ObMsSqlRpcEvent * rpc)
{
  if (NULL != rpc)
  {
...
void ObMsSqlRequestEvent::print_info(FILE * file) const
{
  if (NULL != file)
  {
    // no need lock
    fprintf(file, "merger request event:allocator[%lu], id[%lu], wait[%ld], finish[%ld], succ[%d]\n",
        ObMsSqlRequestEvent::id_allocator_, request_id_, waiting_queue_.size(), finish_queue_.size(),
        result_queue_.size());
...
int ObMsSqlRequestEvent::update_location_cache(const ObServer &svr, const int32_t result_code, const ObScanParam & scan_param)
{
  int err = OB_SUCCESS;
...
int ObMsSqlRequestEvent::update_location_cache(const ObServer &svr, const int32_t result_code, const ObCellInfo & cell)
{
  int err = OB_SUCCESS;
  ObMergerTabletLocation addr;
  addr.server_.chunkserver_ = svr;
...
int ObMsSqlRequestEvent::terminate_remote_session(const common::ObServer& server, const int64_t session_id)
{
...
int ObMsSqlRequestEvent::remove_invalid_event_in_finish_queue(const int64_t timeout)
{
  int ret = OB_SUCCESS;
  int64_t remainder = timeout;
...
int64_t ObMsSqlRequestEvent::get_waiting_queue_size()
{
  return waiting_queue_.size();
}

int64_t ObMsSqlRequestEvent::get_finish_queue_size()
{
  return finish_queue_.size();
}

// TODO
bool ObMsSqlRequestEvent::is_finish()
{
  return finish_;
}
...
int ObMsSqlRequestEvent::wait_single_event(int64_t &timeout)
{
  int ret = OB_SUCCESS;
  int64_t cur_timestamp = tbsys::CTimeUtil::getTime();
  int64_t last_timestamp = cur_timestamp + timeout;
