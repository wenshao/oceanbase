...
#ifndef OB_MERGER_SCAN_EVENT_H_
#define OB_MERGER_SCAN_EVENT_H_
...
namespace oceanbase
{
  namespace mergeserver
  {
    ...
    class ObMergerScanEvent:public ObMergerRequestEvent
    {
    public:
      ObMergerScanEvent(ObMergerLocationCacheProxy * cache_proxy, ObMergerAsyncRpcStub * async_rpc);
      virtual ~ObMergerScanEvent();
      ...
      /// using the iterator for send new sub request for new tablets
      ...
      /// get next packet for this request add a new sub request
      int get_session_next(const int64_t sub_req_idx, const ObMergerRpcEvent &prev_rpc_event,
        common::ObRange &query_range, const int64_t timeout_us,  const int64_t limit_offset = 0);
      ...
      bool    get_is_result_precision() const
      {
    ...
    inline const int32_t ObMergerScanEvent::get_total_sub_request_count() const
    {
      return total_sub_request_count_;
    }

    inline const int32_t ObMergerScanEvent::get_finished_sub_request_count() const
    {
