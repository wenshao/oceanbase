...
namespace oceanbase
{
  namespace mergeserver 
  {
    ...
    {
      //memset(this, 0, sizeof(ObScannerLoader));
      table_id_ = 0;
    ...
    int ObScannerLoader::get_decoded_scanner(ObScanner &param)
    {
      ...
      if (config_loaded_)
      {
        ...
        TBSYS_LOG(DEBUG, "range_start_=%.*s", range_start_.length(), range_start_.ptr());
        TBSYS_LOG(DEBUG, "range_end_=%.*s", range_end_.length(), range_end_.ptr());
    ...
    int ObScannerLoader::build_cell_info(int table_id, ObString row_key, char *buf, ObScanner &param)
    {
      ObCellInfo info;
      ObString tmpstr;
      ...
      TBSYS_LOG(DEBUG, "[%lu][%.*s] [%lu]:[%s]", info.table_id_, 
          info.row_key_.length(), info.row_key_.ptr(), 
          info.column_id_, p);
    ...
    void ObScannerLoader::dump_config()
    {
      ...
      {
        TBSYS_LOG(INFO, "%s => %.*s:%s:%s", OBSP_RANGE_START, range_start_.length(), range_start_.ptr(), 
            range_start_inclusive_?"inclusive":"exclusive",
            range_start_min_?"MIN":""
            );
      }
      ...
      {
      TBSYS_LOG(INFO, "%s => %.*s:%s:%s", OBSP_RANGE_END, range_end_.length(), range_end_.ptr(), 
          range_end_inclusive_?"inclusive":"exclusive",
          range_end_max_?"MAX":""
          );
      }
