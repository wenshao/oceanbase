...
int ObLocationListCacheLoader::get_decoded_location_list_cache(ObMergerTabletLocationCache & cache)
{
  ObRange range;
  ObServer server;
  ObString table_name;
    ...
    cache.init(50000 * 5, 1000, 100000);
    {
      ...
      for (i = 0; i < range_num_; i++)
      {
        ...
        if (strncasecmp(range_start_[i].ptr(), "MIN", 3) == 0)
        {
          ...
          range.start_key_.assign(NULL, 0);
        }
        else
        {
          range.start_key_ = range_start_[i];
        }

        if (strncasecmp(range_end_[i].ptr(), "MAX", 3) == 0)
        {
          range.border_flag_.set_max_value();
          /// range.end_key_.assign(NULL, 0);
          range.end_key_ = fake_max_row_key_;
        }
        else
        {
          range.end_key_ = range_end_[i];
        }
...
void ObLocationListCacheLoader::dump_param(ObScanner &param)
{
  UNUSED(param);
  TBSYS_LOG(INFO, "DUMP ObMergerTabletLocationList");
}
