...
namespace oceanbase
{
  ...
  {
    ObMergerScanEvent::ObMergerScanEvent(ObMergerLocationCacheProxy * cache_proxy, ObMergerAsyncRpcStub * async_rpc)
    :ObMergerRequestEvent(cache_proxy, async_rpc)
    {
      reset();
    }

    ObMergerScanEvent::~ObMergerScanEvent()
    {

    }

    int ObMergerScanEvent::alloc_sub_scan_request(ObMergerSubScanRequest *& sub_req)
    {
      int ret = OB_SUCCESS;
      ...
      {
        TBSYS_LOG(WARN, "allocate sub scan request failed. [total_sub_request_count_=%d]"
            "[finished_sub_request_count_=%d][MAX_SUBREQUEST_NUM=%d]",
            total_sub_request_count_, finished_sub_request_count_, MAX_SUBREQUEST_NUM);
        ret = OB_ERROR;
      }
      return ret;
    }
    ...
    int ObMergerScanEvent::set_request_param(ObMergerScanParam &scan_param, const int64_t max_memory_size,
    ...
    int ObMergerScanEvent::send_rpc_event(ObMergerSubScanRequest * sub_req, const int64_t timeout_us,
      uint64_t * triggered_rpc_event_id)
    {
      ...
      if ((OB_SUCCESS == err) &&
        ...
        (static_cast<double>(timeout_us) * get_timeout_percent()), selected_server.addr_,
        /* @note range and offset set in get_scan_param() function */
        *(sub_req->get_scan_param()), *rpc_event))))
      {
        char addr[MAX_SERVER_ADDR_SIZE] = "";
        selected_server.addr_.to_string(addr, sizeof(addr));
        TBSYS_LOG(WARN, "fail to send scan chunkserver request. [server=%s, err=%d]", addr, err);
        ...
        if (OB_SUCCESS != (tmp_err = ObMergerRequestEvent::destroy(rpc_event)))
        {
    ...
    int ObMergerScanEvent::get_session_next(const int64_t sub_req_idx, const ObMergerRpcEvent &prev_rpc_event,
        ObRange &query_range, const int64_t timeout_us,  const int64_t limit_offset)
    {
      ...
      TBSYS_LOG(INFO, "[session next] timeout_us:%ld, limit_offset:%ld, limit_count:%ld, "
        "total_rpc_event_count:%d, server:%s, session_id:%ld, reqest_id:%lu,"
        "prev_event_id:%lu,cur_event_id:%lu]",  timeout_us, limit_offset, limit_count,
        total_sub_request_count_, prev_rpc_event.get_server().to_cstring(),
        prev_rpc_event.get_session_id(), get_request_id(),
        prev_rpc_event.get_event_id(), (new_rpc_event ? (new_rpc_event->get_event_id()):0));
      return err;
    }

    int ObMergerScanEvent::do_request(const int64_t max_parellel_count,
    ...
    int ObMergerScanEvent::find_sub_scan_request(ObMergerRpcEvent * rpc_event,
    ...
    int ObMergerScanEvent::check_if_need_more_req(const int64_t sub_req_idx,  const int64_t timeout_us,
        ObMergerRpcEvent &prev_rpc_event)
    {
      int err = OB_SUCCESS;
      ...
      if (OB_SUCCESS == err)
      {
        ...
        else
        {
          ...
          else if (prev_rpc_event.is_session_end())
          {
            ...
            TBSYS_LOG(DEBUG, "prev_rpc_event return OB_SESSION_END, end session, no need more request.");
            err = OB_ITER_END;
          }
    ...
    bool ObMergerScanEvent::check_if_location_cache_valid_(const ObScanner & scanner, const ObScanParam & scan_param)
    {
      int err = OB_SUCCESS;
      bool res = true;
     ...
     */
    int ObMergerScanEvent::process_result(const int64_t timeout_us, ObMergerRpcEvent *rpc_event, bool& finish)
    {
      ...
      if ((OB_SUCCESS == err) && (OB_SUCCESS == rpc_event->get_result_code()))
      {
            ...
            && (OB_SUCCESS == rpc_event->get_result_code()))
        {
          ...
          //TBSYS_LOG(INFO, "got first finished rpc event of [sub_request:%ld,us_used:%ld,"
          ...
          if (OB_SUCCESS == err && NULL != scanner && !scanner->get_is_result_precision())
          {
          ...
          if (OB_SUCCESS == err)
          {
            ...
            FILL_TRACE_LOG("finish add cs result[#cells:%ld,#rows:%ld,whole_rows:%ld,"
                ...
                "is_result_precision=%d,last_rowkey:%s, range:%s, scan_param:%s]",
                ...
                ip_addr, is_fullfill, fullfill_num, is_result_precision_, print_string(last_rowkey),
    ...
    void ObMergerScanEvent::end_sessions_()
    {
    ...
    int ObMergerScanEvent::retry(const int64_t sub_req_idx, ObMergerRpcEvent *rpc_event, int64_t timeout_us)
    {
      int err = OB_SUCCESS;
      ...
      if ((OB_SUCCESS == err) && (OB_DATA_NOT_SERVE == rpc_event->get_result_code()))
      {
        ...
        if ((OB_SUCCESS == err)
          ...
          sub_requests_[sub_req_idx].get_scan_param(), &get_buffer_pool()))))
        {
          TBSYS_LOG(WARN,"fail to initialize range iterator [err:%d]", err);
        }
    ...
    int ObMergerScanEvent::prepare_get_cell()
    {
      int err = OB_SUCCESS;
    ...
    int ObMergerScanEvent::next_cell()
    {
      int err = OB_ITER_END;
    ...
    int ObMergerScanEvent::get_cell(ObInnerCellInfo** cell)
    {
      return merger_operator_.get_cell(cell, NULL);
    }

    int ObMergerScanEvent::get_cell(ObInnerCellInfo** cell, bool* is_row_changed)
    {
      int err = OB_ERROR;
    ...
    int ObMergerScanEvent::fill_result(common::ObScanner & scanner, common::ObScanParam &org_scan_param,
      bool &got_all_result)
    {
      int err = OB_SUCCESS;
      ...
      while ((OB_SUCCESS == err) && (!size_over_flow))
      {
        ...
        {
          TBSYS_LOG(WARN,"fail to get next cell from ObMergerScanEvent [err:%d, got_cell_count:%ld]", err, got_cell_count);
        }
        if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = get_cell(&cur_cell, &is_row_changed))))
        {
          TBSYS_LOG(WARN,"fail to get cell from ObMergerScanEvent [err:%d, got_cell_count:%ld]", err, got_cell_count);
        }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          TBSYS_LOG(WARN,"unexpected err [got_cell_count:%ld,result_row_width:%ld]",
              scanner.get_cell_num(), result_row_width);
          err = OB_ERR_UNEXPECTED;
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        scanner.set_is_result_precision(is_result_precision_);
