 ...
 *
 * ob_ms_get_event_parellel.cpp for
 *
 * Authors:
...
ObMergerParellelGetEvent::ObMergerParellelGetEvent(ObMergerLocationCacheProxy * cache_proxy,
  ObMergerAsyncRpcStub * async_rpc): ObMergerRequestEvent(cache_proxy, async_rpc),
...
void ObMergerParellelGetEvent::reset()
{
...
int ObMergerParellelGetEvent::distribute_request()
{
  int err = OB_SUCCESS;
  ObMergerTabletLocationList loc_list;
  loc_list.set_buffer(ObMergerRequestEvent::get_buffer_pool());
  ...
  for (int64_t row_idx = 0; (row_idx < row_count) && (OB_SUCCESS == err); ++row_idx)
  {
      ...
      get_param_->operator [](row_start)->row_key_, loc_list)))
    {
      TBSYS_LOG(WARN,"fail to get tablet location [err:%d,table_id:%lu,rowkey:%.*s]",
        ...
        get_param_->operator [](row_start)->row_key_.length(),
        get_param_->operator [](row_start)->row_key_.ptr());
      break;
    }
    ...
    if ((OB_SUCCESS == err) && (sub_req_idx < 0 || is_subreq_full))
    {
      if (total_sub_request_count_ < MAX_SUBREQUEST_NUM)
      {
        ...
        // start a new sub request for this ip
...
int ObMergerParellelGetEvent::send_request(const int64_t sub_req_idx, const int64_t timeout_us)
{
  int err = OB_SUCCESS;
  ObGetParam *cur_get_param = NULL;
  if ((OB_SUCCESS != (err = sub_requests_[sub_req_idx].get_cur_param(cur_get_param)))
    && (OB_ITER_END != err))
  {
    ...
    (*cur_get_param)[cell_idx]->table_id_, (*cur_get_param)[cell_idx]->row_key_, loc_list))))
  {
    TBSYS_LOG(WARN,"fail to get tablet location [err:%d,table_id:%lu,rowkey:%.*s]", err,
      (*cur_get_param)[cell_idx]->table_id_, (*cur_get_param)[cell_idx]->row_key_.length(),
      (*cur_get_param)[cell_idx]->row_key_.ptr());
  }
  if ((OB_SUCCESS == err) && (0 >= loc_list.size()))
  {
    TBSYS_LOG(WARN,"fail to get tablet location [table_id:%lu,rowkey:%.*s]",
  ...
  for (;(svr_idx < loc_list.size()) && (OB_SUCCESS == err); svr_idx ++)
  {
    if (loc_list[svr_idx].server_.chunkserver_.get_ipv4() == sub_requests_[sub_req_idx].get_last_svr_ipv4())
    {
      if (sub_requests_[sub_req_idx].get_last_svr_ipv4() == sub_requests_[sub_req_idx].get_fail_svr_ipv4())
      {
        ...
        TBSYS_LOG(INFO, "sub req retry [request_id:%lu,svr_idx:%ld,sub_req_idx:%ld,tried_times:%ld,retry_svr:%d, size:%ld]",
          get_request_id(), svr_idx, sub_req_idx, sub_requests_[sub_req_idx].get_retry_times(),
          loc_list[svr_idx].server_.chunkserver_.get_ipv4(), loc_list.size());
      }
      break;
    }
  }
  ...
  if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = ObMergerRequestEvent::create(&rpc_event))))
  {
  ...
  if (OB_SUCCESS == err)
  {
            ...
            loc_list[svr_idx].server_.chunkserver_,(*cur_get_param),*rpc_event)))
    {
      ...
      loc_list[svr_idx].server_.chunkserver_.to_string(addr, sizeof(addr));
      TBSYS_LOG(WARN,"fail to send request to server [request_id:%lu, server:%s, err:%d]",
          get_request_id(), addr, err);
      /// @Exception  get failed, no failure packet would return to the framework
  ...
  ObMergerRequestEvent::get_buffer_pool().reset();
  return err;
}


int ObMergerParellelGetEvent::get_session_next(const int64_t sub_req_idx,
...
int ObMergerParellelGetEvent::check_if_need_more_req(const int64_t current_sub_req_idx,
  const int64_t timeout_us, ObMergerRpcEvent *prev_rpc_event)
{
  ...
  TBSYS_LOG(DEBUG, "send request finish [triggered_sub_request_count_:%d,finished_sub_request_count_:%d,"
    "total_sub_request_count_:%d]", triggered_sub_request_count_, finished_sub_request_count_,
    total_sub_request_count_);
  return err;
}

int ObMergerParellelGetEvent::set_request_param(ObGetParam &get_param,
    const int64_t timeout_us, const int64_t max_parellel_count,
    const int64_t max_get_rows_per_subreq, const int64_t reserve_get_param_count)
{
  int err = OB_SUCCESS;
...
int ObMergerParellelGetEvent::process_result(const int64_t timeout_us, ObMergerRpcEvent *rpc_event, bool& finish)
{
  int err = OB_SUCCESS;
  finish = false;
  ...
  {
    TBSYS_LOG(WARN, "rpc event not belong to current request [request_id:%lu,rpc_event_id:%lu]", get_request_id(),
      rpc_event->get_event_id());
    err = OB_ERR_UNEXPECTED;
  }
  else
  {
    FILL_TRACE_LOG("finish rpc:server:%s,event:%lu,size:%d, err:%d",
      ip_addr, rpc_event->get_event_id(), sub_requests_[sub_req_idx].get_cell_size(), rpc_event->get_result_code());
  }
  if ((OB_SUCCESS == err)
...
int ObMergerParellelGetEvent::reset_iterator()
{
  int err = OB_SUCCESS;
  if (!sealed_)
  {
...
int ObMergerParellelGetEvent::next_cell()
{
  int err = OB_SUCCESS;
  if (sealed_)
  {
...
int ObMergerParellelGetEvent::get_cell(ObInnerCellInfo** cell, bool* is_row_changed)
{
  int err = OB_SUCCESS;
  if (sealed_)
  {
...
int ObMergerParellelGetEvent::get_cell(ObInnerCellInfo** cell)
{
  int err = OB_SUCCESS;
  if (sealed_)
  {
...
int ObMergerParellelGetEvent::fill_result(ObScanner & scanner, ObGetParam &org_param,
  bool &got_all_result)
{
  int err = OB_SUCCESS;
