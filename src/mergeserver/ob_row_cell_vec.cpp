...
oceanbase::mergeserver::ObRowCellVec::ObRowCellVec()
{
  row_buffer_ = NULL;
  row_buffer_size_ = 0;
...
bool oceanbase::mergeserver::ObRowCellVec::is_empty()
{
  return is_empty_;
}

int oceanbase::mergeserver::ObRowCellVec::init()
{
  int err  = OB_SUCCESS;
  if (NULL == row_buffer_)
  {
...
int oceanbase::mergeserver::ObRowCellVec::add_cell(const ObCellInfo & cell)
{
  int err = OB_SUCCESS;
  if (NULL == row_ )
  {
...
void oceanbase::mergeserver::ObRowCellVec::reset_iterator()
{
  consumed_cell_num_ = -1;
}

int oceanbase::mergeserver::ObRowCellVec::next_cell()
{
  int err = OB_SUCCESS;
...
int oceanbase::mergeserver::ObRowCellVec::get_cell(ObCellInfo **cell, bool *is_row_changed)
{
  int err = OB_SUCCESS;
  if (NULL == row_ || 0 == row_->cell_num_)
  {
...
int oceanbase::mergeserver::ObRowCellVec::get_cell(ObCellInfo **cell)
{
  return get_cell(cell,NULL);
}

int64_t oceanbase::mergeserver::ObRowCellVec::get_serialize_size()const
{
  int64_t res = 0;
  int err = OB_SUCCESS;
  if (NULL == row_ || 0 == row_->cell_num_)
...
int oceanbase::mergeserver::ObRowCellVec::serialize(char *buf, const int64_t buf_len,
                                                    int64_t &pos)const
{
  int err = OB_SUCCESS;
...
int oceanbase::mergeserver::ObRowCellVec::deserialize(const char *buf, const int64_t buf_len, int64_t &pos)
{
  int err = OB_SUCCESS;
  if (NULL == row_buffer_)
  {
