...
uint64_t ObMergerRequestEvent::id_allocator_ = 0;

ObMergerRequestEvent::ObMergerRequestEvent(ObMergerLocationCacheProxy * proxy, const ObMergerAsyncRpcStub * rpc)
{
  timeout_ = 0;
  read_param_ = NULL;
  async_rpc_stub_ = rpc;
...
ObMergerRequestEvent::~ObMergerRequestEvent()
{
}

int ObMergerRequestEvent::init(const uint64_t count, const uint32_t mod_id,  ObSessionManager * session_mgr)
{
  int err = OB_SUCCESS;
...
int ObMergerRequestEvent::reset()
{
  timeout_ = 0;
  read_param_ = NULL;
  uint64_t count = 0;
...
uint64_t ObMergerRequestEvent::get_request_id(void) const
{
  return request_id_;
}

const ObMergerAsyncRpcStub * ObMergerRequestEvent::get_rpc(void) const
{
  return async_rpc_stub_;
}

void ObMergerRequestEvent::set_timeout(const int64_t timeout)
{
  timeout_ = timeout;
}

void ObMergerRequestEvent::set_request_param(const ObReadParam * param, const int64_t timeout)
{
  read_param_ = param;
  timeout_ = timeout;
}

int64_t ObMergerRequestEvent::get_timeout(void) const
{
  return timeout_;
}

// create and add a new rpc event to the wait queue
int ObMergerRequestEvent::create(ObMergerRpcEvent ** event)
{
  int ret = OB_SUCCESS;
  if (NULL == event)
  {
...
int ObMergerRequestEvent::destroy(ObMergerRpcEvent * event)
{
  int ret = OB_SUCCESS;
  if (NULL == event)
  {
...
int ObMergerRequestEvent::signal(ObMergerRpcEvent & event)
{
...
const ObReadParam * ObMergerRequestEvent::get_request_param(int64_t & timeout) const
{
  timeout = timeout_;
  return read_param_;
}

void ObMergerRequestEvent::request_used_time(const ObMergerRpcEvent * rpc)
{
  if (rpc != NULL)
  {
...
int ObMergerRequestEvent::process_rpc_event(const int64_t timeout, ObMergerRpcEvent * event, bool & finish)
{
  bool is_valid = false;
  ...
  else if (true == is_valid)
  {
    ...
    {
      TBSYS_LOG(TRACE, "check result client id not equal with current request id:client[%lu], request[%lu], event[%lu]",
          event->get_client_id(), request_id_, event->get_event_id());
    }
  }
  else
  {
    ...
    TBSYS_LOG(TRACE, "the event is not valid:client[%lu], event[%lu], request[%ld], valid[%d]",
...
int ObMergerRequestEvent::remove_wait_queue(ObMergerRpcEvent * event, bool & is_valid)
{
  is_valid = false;
  int ret = OB_SUCCESS;
  if (NULL == event)
...
int ObMergerRequestEvent::wait(const int64_t timeout)
{
  int ret = OB_SUCCESS;
...
int ObMergerRequestEvent::release_succ_event(uint64_t & event_count)
{
  int ret = OB_SUCCESS;
...
int ObMergerRequestEvent::create_rpc_event(ObMergerRpcEvent ** event)
{
  int ret = OB_SUCCESS;
...
void ObMergerRequestEvent::destroy_rpc_event(ObMergerRpcEvent * rpc)
{
  if (NULL != rpc)
  {
...
void ObMergerRequestEvent::print_info(FILE * file) const
{
  if (NULL != file)
  {
    // no need lock
    fprintf(file, "merger request event:allocator[%lu], id[%lu], wait[%ld], finish[%ld], succ[%d]\n",
        ObMergerRequestEvent::id_allocator_, request_id_, waiting_queue_.size(), finish_queue_.size(),
        result_queue_.size());
...
int ObMergerRequestEvent::update_location_cache(const ObServer &svr, const int32_t result_code, const ObScanParam & scan_param)
{
  int err = OB_SUCCESS;
  ObMergerTabletLocation addr;
...
int ObMergerRequestEvent::update_location_cache(const ObServer &svr, const int32_t result_code, const ObCellInfo & cell)
{
  int err = OB_SUCCESS;
  ObMergerTabletLocation addr;
  addr.server_.chunkserver_ = svr;
...
int ObMergerRequestEvent::terminate_remote_session(const common::ObServer& server, const int64_t session_id)
{
...
int ObMergerRequestEvent::remove_invalid_event_in_finish_queue(const int64_t timeout)
{
  int ret = OB_SUCCESS;
  int64_t remainder = timeout;
