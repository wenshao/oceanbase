...
TEST(ObResultSetTest, logical_plan_return)
{
  ...
  const char *sql_str = ""
    ...
    "from (select * from order_list where order_time > date '2012-08-01') o, user "
  ...
  ObMultiPlan* multi_plan = static_cast<ObMultiPlan*>(resultPlan.plan_tree_);
  ObLogicalPlan *logical_plan = multi_plan->at(0);
  ObResultSet result_set;
  ret = logical_plan->fill_result_set(result_set);
  ASSERT_EQ(ret, OB_SUCCESS);
