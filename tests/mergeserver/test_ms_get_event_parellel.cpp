...
public:
  ...
  virtual int get_session_next(const int64_t , const common::ObServer & , 
    const int64_t , int32_t , ObMergerRpcEvent & rpc_event)const
  {
    session_rpc_count_ ++;
    received_rpcs_.push_back(&rpc_event);
    return OB_SUCCESS;
...
struct MockEnv
{
  ...
  ObMergerTabletLocationCache *loc_cache_;
...
MockEnv build_env()
{
  ...
  ObMergerTabletLocationCache *loc_cache  = new ObMergerTabletLocationCache();
...
TEST(ObMergerSubGetRequest, distribute_requests)
{
  ...
  const int64_t cell_count = 1000;
  ObString rowkey;
  rowkey.assign((char*)c_rowkey,static_cast<int32_t>(strlen(c_rowkey)));
  ...
  for (int64_t s_idx = 0; s_idx < sub_req_count; s_idx ++)
  {
    ...
    total += cur_param->get_cell_size();
  }
  EXPECT_EQ(total, cell_count);
  for (int64_t s_idx = 0; s_idx < sub_req_count; s_idx ++)
  {
    ObGetParam *cur_param = NULL;
    EXPECT_EQ(sub_reqs[s_idx].get_cur_param(cur_param) ,OB_SUCCESS);
    ...
    {
      EXPECT_EQ(async_rpc.get_received_rpcs()[s_idx]->get_result().add_cell(*(*cur_param)[cell_idx]), OB_SUCCESS);
    }
    async_rpc.get_received_rpcs()[s_idx]->set_result_code(OB_SUCCESS);
    bool finish = false;
    EXPECT_EQ(get_event.process_result(timeout_us, async_rpc.get_received_rpcs()[s_idx], finish), OB_SUCCESS);
  }
  EXPECT_EQ(get_event.TEST_get_triggered_sub_req_count(), sub_req_count);
  EXPECT_EQ(async_rpc.get_rpc_count(), sub_req_count);
  EXPECT_EQ(async_rpc.get_received_rpcs().size(), (size_t)sub_req_count);
...
TEST(ObMergerSubGetRequest, get_multi_time)
{
  ...
  EXPECT_FALSE(finish);
  EXPECT_EQ(async_rpc.get_rpc_count(), sub_req_count + 1);
  EXPECT_EQ(async_rpc.session_rpc_count(), 0);
...
TEST(ObMergerSubGetRequest, retry)
{
  ...
  EXPECT_FALSE(finish);
  EXPECT_EQ(async_rpc.get_rpc_count(), sub_req_count + 1);
  EXPECT_EQ(async_rpc.get_received_rpcs().size(), (size_t)sub_req_count + 1);
  EXPECT_EQ(async_rpc.session_rpc_count(), 0);
...
TEST(ObMergerSubGetRequest, result_process)
{
  ...
  /// first rpc event finish
  bool finish = false;
  for (int64_t rpc_idx = 0; rpc_idx < sub_req_count; rpc_idx ++)
  {
    int64_t sub_req_idx = 0;
    for (; sub_req_idx < sub_req_count; sub_req_idx ++)
    {
