...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    const int16_t SSTableWriter::KEY_STREAM_MAGIC    = 0x4B73; //"Ks"
    const int16_t SSTableWriter::TRAILER_MAGIC       = 0x5472; //"Tr"

    SSTableWriter::SSTableWriter()
      : inited_(false), first_row_(true), add_row_count_(true),  
                                     ...
                                     int64_t& sstable_size)
    {
      ...
      else
      {
        ...
        TBSYS_LOG(WARN, "No data to write, "
                        "table_id=%lu, row_count=%ld, offset=%ld",
                  table_id_, row_count_, offset_);
      }
    ...
    int SSTableWriter::write_trailer()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          TBSYS_LOG(INFO, "close sstable, checksum=%lu talbe_id=%lu",
                    sstable_checksum_, table_id_);
          hex_dump(cur_key_.ptr(), cur_key_.length(),
