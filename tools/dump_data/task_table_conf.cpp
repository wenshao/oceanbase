...
int RowkeyWrapper::encode(const ObString &rowkey, ObDataBuffer &buff, const RecordDelima &delima) const
{
  ...
  while (itr != end())
  {
    ...
    if (ret != OB_SUCCESS)
    {
      TBSYS_LOG(ERROR, "can't append rowkey item, start_pos=%ld, type=%d",
    ...
    if (ret != OB_SUCCESS)
    {
      TBSYS_LOG(ERROR, "can't append delima, start_pos=%ld, type=%d",
                itr->start_pos, itr->type);
      break;
    }
   ...
   case INT16:
     {
       ...
       {
         TBSYS_LOG(ERROR, "rowkey item config error, start_pos=%ld, type=%d, key_part_len = %d", 
   ...
   case INT8:
     {
       ...
       if (key.length() < (item.start_pos + 1) ||
           item.item_len != 1) 
       {
         TBSYS_LOG(ERROR, "rowkey item config error, start_pos=%ld, type=%d", 
   ...
   case STR:
     {
       if (key.length() < item.start_pos + item.item_len) 
       {
         TBSYS_LOG(ERROR, "configuration error, item.start_pos=%ld, item.item_len=%ld",
   ...
   case VSTR:
     {
       ...
       {
         TBSYS_LOG(ERROR, "configuration error, item.start_pos=%ld",
                   item.start_pos);
         break;
       }
...
int TableConf::revise_column(const ObCellInfo &cell, ObDataBuffer &data) const
{
  ...
  else 
  {
      ...
      int len = hex_to_str(cell.row_key_.ptr(), cell.row_key_.length(), buf, 128);
      buf[2 * len] = 0;
