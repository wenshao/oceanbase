...
{ 
  namespace tools
  {
    ...
    {
      public:
        ObAppStats(ObClientRpcStub &stub, const int64_t server_type, const char* config_file) ;
        virtual ~ObAppStats() {}
      protected:
        virtual int32_t refresh() ;
      private:
        int sum_cluster_stats(const int64_t server_type, const common::ObArrayHelper<common::ObServer>& server_array);
        int sum_cs_cluster_stats(const common::ObServer& rs, const int64_t server_type);
        int sum_ups_cluster_stats(const common::ObServer& rs, const int64_t server_type);
      private:
